...
(function () {
  ...
  try {
    ...
    const file = new File(
      [blob],
      'NebulaBeat Demo Track.mp3',
      { type: 'audio/mpeg' }
    );

    if (window._nbHandleFiles) {
      window._nbHandleFiles([file]);
    }

    playerTitle.textContent = 'NebulaBeat Demo Track';
    playerMode.textContent = 'Demo Mode';
  ...
  document.querySelectorAll('.theme-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      if (btn.dataset.theme === 'custom') {
        $('customThemePopover')?.classList.toggle('open');
        return;
      }
      $('customThemePopover')?.classList.remove('open');
  ...
  const ctpPopover = $('customThemePopover');
  $('ctpApplyBtn')?.addEventListener('click', ()=>{
    const c1 = $('ctpColor1')?.value || '#60a5fa';
    const c2 = $('ctpColor2')?.value || '#f472b6';
    window.NebulaParticles.setCustomTheme(c1, c2);
    document.body.setAttribute('data-theme','custom');
    document.querySelectorAll('.theme-btn').forEach(b=>b.classList.remove('active'));
    $('customThemeBtn')?.classList.add('active');
    ctpPopover?.classList.remove('open');
  });
  document.addEventListener('click', e=>{
    if (!ctpPopover || !ctpPopover.classList.contains('open')) return;
    if (ctpPopover.contains(e.target) || e.target.id === 'customThemeBtn') return;
    ctpPopover.classList.remove('open');
  });
  ...
  canvas.addEventListener('touchstart', e=>{
    ...
    touchStart = startedAt;
    touchBh = false;
    const t = e.touches[0];
    setTimeout(()=>{
      // Only activate if THIS touch is still the active one (not released yet)
      // Compare against startedAt (captured in closure) not touchStart (which
      // gets reset to 0 on touchend — causing false activation on quick taps)
      if (touchStart === startedAt) {
        touchBh = true;
  ...
  // Audio EQ — bass / mid / treble (actually shapes the sound, not just visuals)
  [['eqBassSlider','eqBassVal','bass'],['eqMidSlider','eqMidVal','mid'],['eqTrebleSlider','eqTrebleVal','treble']]
    .forEach(([sliderId,valId,band])=>{
      const slider = $(sliderId), val = $(valId);
      if (!slider) return;
      slider.addEventListener('input',()=>{
        const db = parseInt(slider.value);
        val.textContent = (db>0?'+':'')+db+' dB';
        window.NebulaAudio?.setEQ(band, db);
      });
    });
  $('eqResetBtn')?.addEventListener('click',()=>{
    ['eqBassSlider','eqMidSlider','eqTrebleSlider'].forEach(id=>{ const s=$(id); if(s) s.value=0; });
    ['eqBassVal','eqMidVal','eqTrebleVal'].forEach(id=>{ const v=$(id); if(v) v.textContent='0 dB'; });
    window.NebulaAudio?.resetEQ();
  });
