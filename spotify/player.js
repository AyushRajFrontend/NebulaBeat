...
(function () {
  ...
  function cacheEls() {
    el = {
      ...
      queueList:  document.getElementById('sppQueueList'),
  ...
  async function openQueuePanel() {
    if (!el.queuePanel || !el.queueList) return;
    el.queuePanel.classList.add('open');
    el.queueList.innerHTML = '<div class="spp-queue-loading">Loading…</div>';

    const r = await API.get('/me/player/queue');
    if (!r.ok || !r.data) { el.queueList.innerHTML = '<div class="spp-queue-empty">Couldn\'t load the queue.</div>'; return; }

    const items = (r.data.queue || []).slice(0, 10);
    if (!items.length) { el.queueList.innerHTML = '<div class="spp-queue-empty">Nothing queued.</div>'; return; }

    el.queueList.innerHTML = items.map(t => `
      <div class="spp-queue-item">
        <img src="${t.album?.images?.[2]?.url || t.album?.images?.[0]?.url || ''}" alt="">
        <div class="spp-queue-item-info">
          <div class="spp-queue-item-name">${Utils.escapeHtml(t.name)}</div>
          <div class="spp-queue-item-artist">${Utils.escapeHtml(t.artists.map(a => a.name).join(', '))}</div>
        </div>
      </div>
    `).join('');
  }
  ...
  function init() {
    ...
    Connection.onStateChange((state) => {
      ...
      player.addListener('player_state_changed', s => { if (s) renderTrack(s); });
    });
  }
