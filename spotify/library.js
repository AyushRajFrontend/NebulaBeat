...
(function () {
  ...
  /* ════════════════════════════════════
     FETCH — unified next-URL pagination
  ════════════════════════════════════ */
  ...
  /* ════════════════════════════════════
     SORT / FILTER — client-side, over what's already been loaded
  ════════════════════════════════════ */
  ...
  function renderItems() {
    ...
    el.list.innerHTML = items.map((item, i) => `
      <div class="splib-row" data-index="${i}">
        ...
        ${cat.type === 'track' ? `<button class="splib-queue-btn" data-queue-index="${i}" aria-label="Add to queue">➕</button>` : ''}
      </div>
    `).join('');

    el.list.querySelectorAll('.splib-queue-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const item = items[Number(btn.dataset.queueIndex)];
        if (item) { window.NebulaSpotify.Queue?.addTrack(item); Utils.toast('Added to queue', 'success', { duration: 1500 }); }
      });
    });
  ...
  /* ════════════════════════════════════
     PANEL REGISTRATION — this section joins the generic Search/Library/
     Profile tab switcher in utils.js rather than hardcoding its own
     visibility logic against the other panels.
  ════════════════════════════════════ */
  function onActivate() {
