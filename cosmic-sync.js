...
(function () {
  ...
  function joinSocket(code) {
    ...
    function connect() {
      ...
      SYNC.socket.on('explode',   d => window.NebulaParticles?.explode(d.x, d.y));
      SYNC.socket.on('blackhole', d => window.NebulaParticles?.setBlackHole(d.x, d.y, d.active));
      SYNC.socket.on('beat',      d => window.NebulaParticles?.hitBeat(d.strength));
      ...
      SYNC.socket.on('connect_error', () => {
        updateStatus('⚠ Could not reach server. Update SERVER_URL in cosmic-sync.js');
        updateBadge('ERROR');
      });
  ...
  function handleBroadcast(msg) {
    ...
    switch (msg.type) {
      ...
      case 'EXPLODE':    window.NebulaParticles?.explode(msg.x, msg.y); break;
      case 'BLACKHOLE':  window.NebulaParticles?.setBlackHole(msg.x, msg.y, msg.active); break;
      case 'BEAT':       window.NebulaParticles?.hitBeat(msg.strength); break;
  ...
  function patchParticles() {
    ...
    NP.explode      = (x,y)          => { orig.explode(x,y);          emit('explode',    {x,y}); };
    NP.setBlackHole = (x,y,active)   => { orig.setBlackHole(x,y,active); emit('blackhole', {x,y,active}); };
    NP.hitBeat      = (str)          => { orig.hitBeat(str);           emit('beat',       {strength:str}); };
    NP.setTheme     = (t)            => { orig.setTheme(t);            emit('theme',      {theme:t}); };
    NP.setScene     = (s)            => { orig.setScene(s);            emit('scene',      {scene:s}); };
  }
