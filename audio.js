...
(function () {
  ...
  recordDestination: null,
    ...
    // 3-band EQ (persistent filter chain)
  ...
  function ensureCtx() {
    if (!A.ctx) {
      ...
      // 3-band EQ: every source connects into eqLow; the chain fans out
      // to both the analyser (→ speakers) and the shareable record stream,
      // so what you see, hear, and share are all consistently EQ'd.
      A.eqLow  = A.ctx.createBiquadFilter();
      A.eqLow.type = 'lowshelf';
      A.eqLow.frequency.value = 320;
      A.eqLow.gain.value = 0;

      A.eqMid  = A.ctx.createBiquadFilter();
      A.eqMid.type = 'peaking';
      A.eqMid.frequency.value = 1000;
      A.eqMid.Q.value = 0.9;
      A.eqMid.gain.value = 0;

      A.eqHigh = A.ctx.createBiquadFilter();
      A.eqHigh.type = 'highshelf';
      A.eqHigh.frequency.value = 3200;
      A.eqHigh.gain.value = 0;

      A.eqLow.connect(A.eqMid);
      A.eqMid.connect(A.eqHigh);
      A.eqHigh.connect(A.analyser);
      A.eqHigh.connect(A.recordDestination);
  ...
  return A.recordDestination
    ...
    // 3-band EQ — gain in dB, roughly -12..+12
    setEQ(band, db) {
      ensureCtx();
      const node = band === 'bass' ? A.eqLow : band === 'mid' ? A.eqMid : band === 'treble' ? A.eqHigh : null;
      if (!node) return;
      node.gain.value = Math.max(-12, Math.min(12, db));
    },
    getEQ() {
      return {
        bass:   A.eqLow  ? A.eqLow.gain.value  : 0,
        mid:    A.eqMid  ? A.eqMid.gain.value  : 0,
        treble: A.eqHigh ? A.eqHigh.gain.value : 0
      };
    },
    resetEQ() {
      ensureCtx();
      [A.eqLow, A.eqMid, A.eqHigh].forEach(n => { if (n) n.gain.value = 0; });
    },
