...
(function () {
  ...
  /* ── Highlight auto-capture state ── */
  const HL = {
    enabled:      false,
    watching:     false,
    cooldownUntil: 0,
    threshold:    1.05,    // beat strength (0–1.4) that counts as a "drop"
    clipSeconds:  12,
    cooldownMs:   30000
  };
  ...
  /* ════════════════════════════════════
     ...
     Watches the live beat level and, on a
     strong "drop", automatically records a
     short clip and exports it — no manual
     button press needed.
     NOTE: captures live FROM the moment the
     drop is detected onward (browsers can't
     retroactively rewind audio/video without
     a constantly-running pre-buffer, which
     risks producing corrupt/unplayable clips —
     so this trades a few seconds of lead-in
     for a clip that's guaranteed to play).
  ════════════════════════════════════ */
  function captureHighlight() {
    if (REC.isRecording) return; // a recording (manual or highlight) is already running
    HL.cooldownUntil = Date.now() + HL.cooldownMs;
    REC._isHighlight = true;
    startRecording();
    setTimeout(() => {
      if (REC.isRecording && REC._isHighlight) stopRecording();
    }, HL.clipSeconds * 1000);
  }

  function highlightWatchLoop() {
    if (!HL.enabled) { HL.watching = false; return; }
    HL.watching = true;
    const level = window.NebulaParticles?.getBeatLevel?.() ?? 0;
    if (level >= HL.threshold && Date.now() >= HL.cooldownUntil && !REC.isRecording) {
      captureHighlight();
    }
    requestAnimationFrame(highlightWatchLoop);
  }

  function setHighlightEnabled(on) {
    HL.enabled = !!on;
    if (HL.enabled && !HL.watching) requestAnimationFrame(highlightWatchLoop);
  }
  ...
  function showExportModal(blob, blobUrl, mime) {
    ...
    const eyebrowEl = document.getElementById('exportEyebrow');
    const titleEl   = document.getElementById('exportTitle');
    if (eyebrowEl) eyebrowEl.textContent = REC._isHighlight ? '🔥 AUTO-DETECTED' : 'COSMIC SESSION';
    if (titleEl)   titleEl.textContent   = REC._isHighlight ? 'Highlight Captured!' : 'Export Complete';
    eyebrowEl?.classList.toggle('highlight-eyebrow', !!REC._isHighlight);
    REC._isHighlight = false;
  ...
  function bindButton() {
    ...
    if (hlToggle) {
      hlToggle.addEventListener('change', () => setHighlightEnabled(hlToggle.checked));
    }
  }
